...
function saveInventory(){
  ...
  try{ saveCloudInventory(); }catch(e){} // no-op unless signed in to Supabase
}
loadInventory();
...
class Ball{
  ...
  update(dt, quiet){
    // quiet=true is used by the netcode to fast-forward / extrapolate the ball
    // without sounds, particles or trail points
    ...
    // side walls (y)
    if(this.y < this.r){ this.y=this.r; this.vy = Math.abs(this.vy)*0.78; if(!quiet){ Audio1.wallThud(); burst(this.x,this.y,4,'rgba(255,255,255,.5)',100,.2,2,false); } }
    if(this.y > WORLD.h-this.r){ this.y=WORLD.h-this.r; this.vy = -Math.abs(this.vy)*0.78; if(!quiet) Audio1.wallThud(); }

    const inGoalMouthY = this.y > goalYMin()+this.r*0.4 && this.y < goalYMax()-this.r*0.4;
    if(!inGoalMouthY){
      if(this.x < this.r){ this.x=this.r; this.vx = Math.abs(this.vx)*0.78; if(!quiet) Audio1.wallThud(); }
      if(this.x > WORLD.w-this.r){ this.x=WORLD.w-this.r; this.vx = -Math.abs(this.vx)*0.78; if(!quiet) Audio1.wallThud(); }
    } else {
      // back of net
...
function carBallCollision(car, ball){
  ...
  if(zDiff > 46) return false; // too far apart vertically (car mostly grounded car vs high ball)
  ...
  return true; // the netcode uses this to report touches
}
...
function setupMatch(mode){
  ...
  cancelMatchmaking();
  if(net) cleanupOnlineMatch(true); // leaving an online match/search to play local
  game.mode = mode;
  LOCAL_TEAM = 'blue';
...
$('#btnSignupSubmit').addEventListener('click', async ()=>{
  ...
  // the auth listener's profile read can land before this write, so apply the name directly
  localName = name;
  updateAccountChip();
  showOnly(menuMain);
});
...
function doLogout(){
  const wasOnline = !!(net || lobby);
  cancelMatchmaking();
  if(net) cleanupOnlineMatch(true);
  if(cloudReady) sb.auth.signOut();
  authUser = null; localName = 'Guest';
  updateAccountChip();
  if(wasOnline) goToMenu();
}
if(cloudReady){
...
function saveCloudInventory(){
  ...
  // supabase-js query builders are lazy: nothing is sent until the builder is
  // awaited or .then()'d. This call used to do neither, so it never ran.
  sb.from('inventories')
    .upsert({ id: authUser.id, owned: inventory.owned, equipped: inventory.equipped, updated_at: new Date().toISOString() })
    .then(({error})=>{ if(error) console.warn('Cloud inventory save failed:', error.message); }, ()=>{});
}

/* ---- Online matchmaking + realtime match sync ----
   Everything travels over Supabase Realtime; no database rows are needed.

   LOBBY: each searching player joins one shared presence channel under a
   fresh per-search queue id (qid). Every client sorts the visible qids the
   same way and pairs neighbours (1st+2nd, 3rd+4th, ...). The lower qid of a
   pair sends an "invite" carrying a brand-new match id, and the other side
   answers "accept" (or "decline" if it is busy). Nobody leaves the lobby
   until that handshake completes, so a player can no longer walk off into a
   match their opponent never saw.

   MATCH: both players join a channel unique to that match id. The match
   starts as soon as either side sees the other (presence, a "hello", or a
   state packet). The host (inviter, blue) owns the clock, score, goals,
   ball and boost pads. The guest (red) predicts the ball locally and is the
   authority for its own car's touches on the ball, which it reports to the
   host. Remote cars are dead-reckoned from their last packet using a
   measured round-trip time, so they glide instead of jumping between
   packets. */
const LOBBY_TOPIC = 'rr-lobby-v2';        // versioned so stale cached clients can't pair with this protocol
const NET_SEND_MS = 80;                    // regular state packets, ~12/sec per player
const NET_URGENT_GAP_MS = 50;              // a ball touch may send early, but never above 20 packets/sec
const NET_EXTRAP_MAX_S = 0.25;             // never dead-reckon a remote car further than this past its last packet
const INVITE_TIMEOUT_MS = 4000;
const MATCH_CONNECT_TIMEOUT_MS = 10000;
const OPP_LEAVE_GRACE_MS = 2500;           // a presence "leave" must persist this long (with no packets) to count
const OPP_SILENCE_MS = 8000;               // no packets at all for this long means the opponent is gone
const HIT_REPEAT_MS = 400;                 // the guest repeats its latest ball touch in packets for this long

let netRole = null;           // 'host' | 'guest' | null
let net = null;               // live match session (see beginOnlineMatch); null otherwise
let lobby = null;             // live matchmaking session (see startMatchmaking); null otherwise
let onlineOppName = 'Opponent';
let matchStartFailures = 0;
const netScratchBall = new Ball();
...
function chanSend(ch, event, payload){
  // Only push over an open socket. On a channel that isn't joined yet,
  // supabase-js quietly falls back to one HTTP request per send(), which at
  // packet rates means a flood of REST calls and console warnings.
...
// supabase-js returns the EXISTING channel object when asked for a topic that
// is still in its list, including one that is halfway through leaving, and
// subscribe() on that object silently does nothing. Wait for any old copy of
// the topic to finish closing before creating a fresh one.
async function freshChannel(topic, opts){
  const full = 'realtime:' + topic;
  for(let i=0; i<30; i++){
    const stale = (sb.getChannels ? sb.getChannels() : []).find(c=> c.topic===full);
    if(!stale) break;
    if(i===0) releaseChannel(stale);
    await sleep(100);
  }
...
$('#btnCancelLobby').addEventListener('click', ()=>{ Audio1.click(); goToMenu(); });
window.addEventListener('pagehide', ()=>{
  // best effort: tell the opponent right away instead of making them wait for a timeout
  if(net) cleanupOnlineMatch(true);
  cancelMatchmaking();
});

/* ---- Lobby ---- */
async function startMatchmaking(statusText){
  cancelMatchmaking();
  if(net) cleanupOnlineMatch(true);
  if(!cloudReady || !authUser){ showOnly(menuLogin); return; }
  game.mode = 'online';
  game.state = STATE.MENU;
  stopGaragePreviewLoop();
  showOnly(menuLobby);
  const idleText = statusText || 'Searching for an opponent\u2026';
  $('#lobbyStatus').textContent = idleText;
  $('#btnCancelLobby').classList.remove('hidden');

  const L = { qid:newNetId(), uid:authUser.id, name:localName || 'Player', idleText, channel:null, poll:null, pending:null, committed:false, dead:false };
  lobby = L;
  const ch = await freshChannel(LOBBY_TOPIC, { config:{ presence:{ key:L.qid }, broadcast:{ self:false, ack:false } } });
  if(lobby !== L){ releaseChannel(ch); return; }   // cancelled while we were waiting
  L.channel = ch;
  ch.on('presence', { event:'sync' }, ()=> lobbyTick(L));
  ch.on('broadcast', { event:'invite' },  ({payload})=> onLobbyInvite(L, payload));
  ch.on('broadcast', { event:'accept' },  ({payload})=> onLobbyAccept(L, payload));
  ch.on('broadcast', { event:'decline' }, ({payload})=> onLobbyDecline(L, payload));
  ch.subscribe(async (status)=>{
    if(L.dead) return;
    if(status === 'SUBSCRIBED'){
      // This also fires again after an automatic reconnect. Presence is not
      // restored on its own after a reconnect, so always (re)announce here.
      try{ await ch.track({ qid:L.qid, uid:L.uid, name:L.name }); }catch(e){}
      if(L.dead) return;
      if(!L.poll) L.poll = setInterval(()=> lobbyTick(L), 1500);
      if(!L.pending && !L.committed) $('#lobbyStatus').textContent = idleText;
      lobbyTick(L);
    } else if(status === 'CHANNEL_ERROR' || status === 'TIMED_OUT'){
      if(!L.committed) $('#lobbyStatus').textContent = 'Connection trouble \u2014 retrying\u2026';
    }
  });
}
function cancelMatchmaking(){
  ...
  if(L.channel){ releaseChannel(L.channel); L.channel = null; }
}
function lobbyTick(L){
  if(L !== lobby || L.dead || L.committed || !L.channel) return;
  const now = performance.now();
  if(L.pending && now - L.pending.at > INVITE_TIMEOUT_MS){   // unanswered: free to try again
    L.pending = null;
    $('#lobbyStatus').textContent = L.idleText;
  }
  if(L.pending) return;
  const state = L.channel.presenceState();
  const metas = [];
  for(const key in state){
    const m = state[key] && state[key][0];
    if(m && m.qid && m.uid) metas.push(m);
  }
  metas.sort((a,b)=> a.qid < b.qid ? -1 : (a.qid > b.qid ? 1 : 0));
  // one queue slot per account, so two tabs of the same login never pair up
  const seen = new Set(), queue = [];
  for(const m of metas){ if(!seen.has(m.uid)){ seen.add(m.uid); queue.push(m); } }
  const i = queue.findIndex(m=> m.qid === L.qid);
  if(i < 0 || i % 2 === 1) return;   // not visible yet, a duplicate tab, or an odd slot waiting for its invite
  const partner = queue[i+1];
  if(!partner) return;
  L.pending = { to:partner.qid, uid:partner.uid, name:partner.name, matchId:newNetId(), at:now };
  chanSend(L.channel, 'invite', { to:partner.qid, from:L.qid, fromUid:L.uid, fromName:L.name, matchId:L.pending.matchId });
  $('#lobbyStatus').textContent = 'Found ' + (partner.name || 'a player') + ' \u2014 connecting\u2026';
}
function onLobbyInvite(L, p){
  if(L !== lobby || L.dead || !p || p.to !== L.qid) return;
  if(L.committed || L.pending || p.fromUid === L.uid){
    chanSend(L.channel, 'decline', { to:p.from, from:L.qid, matchId:p.matchId });
    return;
  }
  L.committed = true;
  chanSend(L.channel, 'accept', { to:p.from, from:L.qid, fromUid:L.uid, fromName:L.name, matchId:p.matchId });
  cancelMatchmaking();   // the leave is queued behind the accept on the same socket, so the accept still arrives
  beginOnlineMatch({ matchId:p.matchId, role:'guest', oppName:p.fromName, oppUid:p.fromUid });
}
function onLobbyAccept(L, p){
  if(L !== lobby || L.dead || !p || p.to !== L.qid) return;
  // A late accept for an invite we already gave up on is ignored; that player
  // times out of the empty match channel and requeues automatically.
  if(L.committed || !L.pending || L.pending.matchId !== p.matchId) return;
  L.committed = true;
  const info = { matchId:p.matchId, role:'host', oppName:p.fromName || L.pending.name, oppUid:p.fromUid || L.pending.uid };
  cancelMatchmaking();
  beginOnlineMatch(info);
}
function onLobbyDecline(L, p){
  if(L !== lobby || L.dead || !p || p.to !== L.qid) return;
  if(L.pending && L.pending.matchId === p.matchId){
    L.pending = null;
    setTimeout(()=> lobbyTick(L), 250 + Math.random()*500);   // jitter so two retrying players don't collide again
  }
}

/* ---- Match session ---- */
function beginOnlineMatch(info){
  if(net) cleanupOnlineMatch(true);
  stopGaragePreviewLoop();
  netRole = info.role;
  onlineOppName = info.oppName || 'Opponent';
  ...
  particles = []; fxList = [];
  game.score.blue = 0; game.score.red = 0;
  game.timeLeft = 120; game.countdown = 3;
  game.overtime = false; game.winner = null; game.dropAvailable = false;
  game.mode = 'online';
  game.state = STATE.MENU;   // nothing simulates until both players are confirmed connected
  camera.mode = 'car';
  showOnly(menuLobby);
  $('#lobbyStatus').textContent = 'Connecting to ' + onlineOppName + '\u2026';
  $('#btnCancelLobby').classList.remove('hidden');   // stays cancellable while connecting

  const uid = authUser.id;
  const S = net = {
    id: info.matchId, role: info.role, uid,
    channel: null, started: false, over: false, dead: false, timers: [], helloTimer: null,
    sendSeq: 0, lastSendAt: -1e9, peerSeq: -1,
    lastPacketAt: performance.now(), leaveAt: 0,
    rtt: 150, peerT: 0, peerTAt: 0,          // round-trip estimate (ms) and the peer's last timestamp to echo
    remote: null, remoteAt: 0,               // opponent car snapshot + local receive time
    // host only
    hitAck: 0, pendingHit: null, padAck: 0, lastHitFxAt: 0,
    // guest only
    hostSnap: null, hostSnapAt: 0, hitSeq: 0, hitAt: 0, lastHit: null, predictUntil: 0,
    padSeq: 0, padLog: [], padLocalAt: {}, seenScoreB: 0, seenScoreR: 0, seenCd: null,
  };
  const alive = ()=> net === S && !S.dead;
  const ch = sb.channel('rr-match-' + S.id, { config:{ broadcast:{ self:false, ack:false }, presence:{ key:uid } } });
  S.channel = ch;
  ch.on('broadcast', { event: S.role === 'host' ? 'guest_state' : 'host_state' }, ({payload})=>{ if(alive()) onPeerState(S, payload); });
  ch.on('broadcast', { event:'hello' }, ()=>{ if(alive()){ S.lastPacketAt = performance.now(); startOnlineMatch(S); } });
  ch.on('broadcast', { event:'bye' }, ()=>{ if(alive()) opponentGone(S, 'left'); });
  ch.on('presence', { event:'sync' }, ()=>{ if(alive()) onMatchPresence(S); });
  ch.on('presence', { event:'leave' }, ()=>{ if(alive()) onMatchPresence(S); });
  ch.subscribe(async (status)=>{
    if(!alive()) return;
    if(status === 'SUBSCRIBED'){
      try{ await ch.track({ uid, role:S.role }); }catch(e){}   // re-runs after a reconnect, restoring presence
      if(!alive()) return;
      if(!S.started){
        chanSend(ch, 'hello', { role:S.role });
        if(!S.helloTimer) S.helloTimer = setInterval(()=>{ if(alive() && !S.started) chanSend(ch, 'hello', { role:S.role }); }, 500);
      }
    } else if(status === 'CHANNEL_ERROR' || status === 'TIMED_OUT'){
      // Before kickoff: give up on this opponent and requeue. Mid-match,
      // supabase-js rejoins automatically, and the silence timeout in
      // netTick() ends the match only if the connection never comes back.
      if(!S.started) matchStartFailed(S);
    }
  });
  S.timers.push(setTimeout(()=>{ if(alive() && !S.started) matchStartFailed(S); }, MATCH_CONNECT_TIMEOUT_MS));
}
function onMatchPresence(S){
  const st = S.channel ? S.channel.presenceState() : {};
  const oppHere = Object.keys(st).some(k=> k !== S.uid);
  if(oppHere){
    S.leaveAt = 0;
    if(!S.started) startOnlineMatch(S);
  } else if(S.started && !S.leaveAt){
    S.leaveAt = performance.now();   // might be a reconnect blip, so netTick() waits out a grace period first
  }
}
function startOnlineMatch(S){
  if(S.started || S.dead) return;
  S.started = true;
  S.lastPacketAt = performance.now();
  matchStartFailures = 0;
  if(S.helloTimer){ clearInterval(S.helloTimer); S.helloTimer = null; }
  chanSend(S.channel, 'hello', { role:S.role });   // one more, in case the opponent is still waiting on us
  try{ if(document.activeElement && document.activeElement.blur) document.activeElement.blur(); }catch(e){}
  showOnly(null);
  updateHUDVisibility();
  if(S.role === 'host'){
    beginCountdown();                 // the host owns the match clock
  } else {
    game.state = STATE.COUNTDOWN;     // host packets drive the countdown from here
    $('#goalMsg').classList.add('hidden');
  }
}
function matchStartFailed(S){
  if(net !== S) return;
  cleanupOnlineMatch(true);
  matchStartFailures++;
  if(matchStartFailures >= 3){
    matchStartFailures = 0;
    showNotice('Connection Problem', 'Couldn\u2019t connect to an opponent after several tries. Check your connection and try again.');
    return;
  }
  startMatchmaking('Couldn\u2019t reach that player \u2014 searching again\u2026');
}
function opponentGone(S, reason){
  if(net !== S || S.dead) return;
  if(!S.started){ matchStartFailed(S); return; }
  if(S.over){ cleanupOnlineMatch(false); return; }   // result already decided; they just left the results screen
  cleanupOnlineMatch(false);
  game.state = STATE.MENU;   // stop the match here instead of letting it keep simulating behind the notice
  $('#countdownNum').classList.add('hidden');
  $('#goalMsg').classList.add('hidden');
  showNotice('Opponent Disconnected', reason === 'left' ? 'The other player left the match.' : 'Lost the connection to the other player.');
}
function cleanupOnlineMatch(sayBye){
  const S = net;
  net = null; netRole = null; LOCAL_TEAM = 'blue';
  if(!S) return;
  S.dead = true;
  S.timers.forEach(t=> clearTimeout(t));
  if(S.helloTimer){ clearInterval(S.helloTimer); S.helloTimer = null; }
  if(S.channel){
    if(sayBye && !S.over) chanSend(S.channel, 'bye', {});   // sent before the leave on the same socket
    releaseChannel(S.channel);
    S.channel = null;
  }
}

/* ---- Packets ---- */
function netMatchState(){
  switch(game.state){
...
function packCar(car){
  // Outside live play a car is frozen in place, so report zero velocity;
  // otherwise the other side would dead-reckon it drifting away.
  const live = game.state === STATE.PLAYING || game.state === STATE.OVERTIME;
  return {
    x:r1(car.x), y:r1(car.y), z:r1(car.z), a:Math.round(car.angle*1000)/1000,
    vx: live ? r1(car.vx) : 0, vy: live ? r1(car.vy) : 0, vz: live ? r1(car.vz) : 0,
    av: live ? Math.round((car.angVel||0)*1000)/1000 : 0,
    b: Math.round(car.boost), bo: car.boosting ? 1 : 0, sp: Math.round((car.spin||0)*100)/100,
  };
}
function netPublish(S, urgent, force){
  if(!S || S.dead || !S.started || !S.channel) return;
  const now = performance.now();
  if(!force && now - S.lastSendAt < (urgent ? NET_URGENT_GAP_MS : NET_SEND_MS)) return;
  S.lastSendAt = now;
  const me = (S.role === 'host') ? carBlue : carRed;
  const p = {
    s: ++S.sendSeq,
    pt: Math.round(now*10)/10,                              // our clock...
    pe: S.peerT || 0,                                       // ...their clock, echoed back...
    ph: S.peerT ? Math.round(now - S.peerTAt) : 0,          // ...and how long we held it (gives RTT, no clock sync needed)
    car: packCar(me),
  };
  if(S.role === 'host'){
    p.ball = { x:r1(ball.x), y:r1(ball.y), z:r1(ball.z), vx:r1(ball.vx), vy:r1(ball.vy), vz:r1(ball.vz) };
    p.sb = game.score.blue; p.sr = game.score.red;
    p.ms = netMatchState(); p.cd = game.countdown; p.tl = r1(game.timeLeft); p.ot = game.overtime ? 1 : 0;
    p.ha = S.hitAck;                                        // latest guest touch we've processed
    p.bp = boostPads.map(pd=> pd.active ? 0 : Math.max(0.1, r1(pd.cd)));
  } else {
    if(S.lastHit && now - S.lastHit.at < HIT_REPEAT_MS) p.hit = S.lastHit.d;
    const cutoff = now - 600;
    S.padLog = S.padLog.filter(e=> e.at >= cutoff);
    if(S.padLog.length) p.pads = S.padLog.map(e=> ({ q:e.q, i:e.i }));
  }
  chanSend(S.channel, S.role === 'host' ? 'host_state' : 'guest_state', p);
}
function onPeerState(S, p){
  if(!p || typeof p.s !== 'number') return;
  const now = performance.now();
  S.lastPacketAt = now;
  S.leaveAt = 0;   // packets prove they're still here, whatever presence says
  if(!S.started) startOnlineMatch(S);
  if(p.s <= S.peerSeq) return;   // duplicate or out of order
  S.peerSeq = p.s;
  if(p.pe){
    const sample = now - p.pe - (p.ph||0);
    if(sample > 0 && sample < 3000) S.rtt = S.rtt*0.8 + sample*0.2;
  }
  S.peerT = p.pt; S.peerTAt = now;
  if(p.car){ S.remote = p.car; S.remoteAt = now; }
  if(S.role === 'host') hostReceive(S, p, now); else guestReceive(S, p, now);
}
function netTick(S){
  const now = performance.now();
  if(!S.over){
    const leftForGood = S.leaveAt && now - S.leaveAt > OPP_LEAVE_GRACE_MS && now - S.lastPacketAt > 1500;
    if(leftForGood || now - S.lastPacketAt > OPP_SILENCE_MS){
      opponentGone(S, S.leaveAt ? 'left' : 'lost');
      return false;
    }
  }
  netPublish(S, false, false);
  return true;
}

/* ---- Shared helpers ---- */
function setBallFromNet(b, s, clearTrail){
  b.x = s.x; b.y = s.y; b.z = s.z; b.vx = s.vx; b.vy = s.vy; b.vz = s.vz; b.frozen = false;
  if(clearTrail) b.trail.length = 0;
}
function extrapolateBall(s, t){
  // run the real ball physics forward (walls, bounces, friction) without sounds/particles
  const b = netScratchBall;
  setBallFromNet(b, s, false);
  t = Math.min(Math.max(t, 0), 0.4);
  while(t > 1e-4){ const h = Math.min(1/120, t); b.update(h, true); t -= h; }
  return b;
}
function driveRemoteCar(S, car, dt){
  const r = S.remote;
  if(!r) return;
  const now = performance.now();
  // where the opponent most likely is right now: last packet + (age + one-way latency)
  const t = Math.min(NET_EXTRAP_MAX_S, (now - S.remoteAt)/1000 + S.rtt/2000);
  const tx = Math.max(-WORLD.goalDepth+30, Math.min(WORLD.w+WORLD.goalDepth-30, r.x + r.vx*t));
  const ty = Math.max(22, Math.min(WORLD.h-22, r.y + r.vy*t));
  const tz = Math.max(0, r.z + r.vz*t - 750*t*t);
  const ta = r.a + r.av*t;
  if(Math.hypot(tx-car.x, ty-car.y) > 220){
    car.x = tx; car.y = ty; car.angle = ta;   // teleport-sized gap (kickoff reset, first packet): snap
  } else {
    // glide at the reported velocity, then ease out whatever error remains
    car.x += r.vx*dt; car.y += r.vy*dt; car.angle += r.av*dt;
    const k = Math.min(1, 10*dt);
    car.x += (tx-car.x)*k; car.y += (ty-car.y)*k; car.angle += angleDelta(ta, car.angle)*k;
  }
  car.z = tz; car.vx = r.vx; car.vy = r.vy; car.vz = r.vz; car.angVel = r.av;
  car.boost = r.b; car.boosting = !!r.bo; car.grounded = tz <= 0.5; car.spin = car.grounded ? 0 : (r.sp||0);
  if(car.boosting && (game.state === STATE.PLAYING || game.state === STATE.OVERTIME)){
    // the same exhaust particles a locally simulated car emits
    const fx = Math.cos(car.angle), fy = Math.sin(car.angle);
    spawnParticle({
      x:car.x-fx*30, y:car.y-fy*30, vx:-fx*160+(Math.random()-0.5)*60, vy:-fy*160+(Math.random()-0.5)*60,
      life:0.22, maxLife:0.22, color: car.team==='blue' ? 'rgba(120,200,255,0.85)' : 'rgba(255,140,110,0.85)',
      size:6+Math.random()*3, drag:1.2, grav:false
    });
  }
}
function showOnlineResult(b, r){
...
/* ---- Host side ---- */
function hostReceive(S, p, now){
  if(p.hit && p.hit.q > S.hitAck){
    S.hitAck = p.hit.q;
    S.pendingHit = { h:p.hit, at:now };
  }
  if(Array.isArray(p.pads)){
    for(const e of p.pads){
      if(!e || e.q <= S.padAck) continue;
      S.padAck = e.q;
      const pad = boostPads[e.i];
      if(pad && pad.active){
...
function hostApplyGuestHit(S){
  const ph = S.pendingHit;
  if(!ph) return;
  S.pendingHit = null;
  if(performance.now() - ph.at > 400) return;   // arrived during a goal/countdown pause: too stale to replay at kickoff
  const h = ph.h;
  // If our ball is nowhere near where the guest touched it, the host hit it
  // first. Keep ours; the guest's view snaps to it once this is acknowledged.
  if(Math.hypot(ball.x-h.x, ball.y-h.y) > 320) return;
  setBallFromNet(ball, h, false);
  // their touch happened roughly one-way-latency ago: fast-forward it to "now"
  let t = Math.min(0.3, S.rtt/2000 + (performance.now()-ph.at)/1000);
  while(t > 1e-4){ const s = Math.min(1/120, t); ball.update(s, true); t -= s; }
  const now = performance.now();
  if(now - S.lastHitFxAt > 200){
    S.lastHitFxAt = now;
    Audio1.hit(Math.min(1, Math.hypot(h.vx, h.vy)/900));
    burst(ball.x, ball.y, 10, 'rgba(255,140,110,.9)', 220, 0.3, 3, false);
  }
}

/* ---- Guest side ---- */
function guestReceive(S, p, now){
  S.hostSnap = p; S.hostSnapAt = now;
  const sB = p.sb|0, sR = p.sr|0;
  if(sB > S.seenScoreB || sR > S.seenScoreR) celebrateGoal(sB > S.seenScoreB ? 'blue' : 'red');
  S.seenScoreB = sB; S.seenScoreR = sR;
  game.score.blue = sB; game.score.red = sR;
  if(typeof p.tl === 'number') game.timeLeft = p.tl;
  game.overtime = !!p.ot;
  if(Array.isArray(p.bp)) guestMirrorPads(S, p.bp, now);

  const prev = game.state;
  const numEl = $('#countdownNum');
  if(p.ms === 'countdown'){
    if(prev !== STATE.COUNTDOWN) guestKickoffReset(S, p);
    game.state = STATE.COUNTDOWN;
    if(p.cd !== S.seenCd){ S.seenCd = p.cd; Audio1.countdownBeep(p.cd <= 0); }
    numEl.classList.remove('hidden');
    if(p.cd > 0){ numEl.textContent = p.cd; numEl.style.color = 'var(--accent)'; }
  ...
  if(p.ms === 'goal'){
    ...
    if(!S.over){
      S.over = true;
      showOnlineResult(sB, sR);
      cleanupOnlineMatch(false);
    }
  } else {
    game.state = (p.ms === 'overtime') ? STATE.OVERTIME : STATE.PLAYING;
    $('#goalMsg').classList.add('hidden');
  }
}
function guestKickoffReset(S, p){
  if(p && p.ball) setBallFromNet(ball, p.ball, true);   // ball straight to the kickoff spot, not left in the net
  const sp = startPositions();
  carRed.x = sp.red.x; carRed.y = sp.red.y; carRed.angle = sp.red.a;
  carRed.vx = 0; carRed.vy = 0; carRed.vz = 0; carRed.z = 0; carRed.angVel = 0;
  carRed.jumpsLeft = 2; carRed.boosting = false; carRed.spin = 0; carRed.spinVel = 0;
  S.predictUntil = 0;
  $('#goalMsg').classList.add('hidden');
}
function guestMirrorPads(S, bp, now){
  for(let i=0; i<boostPads.length && i<bp.length; i++){
    const pad = boostPads[i], cd = +bp[i] || 0;
    const mineRecently = S.padLocalAt[i] && now - S.padLocalAt[i] < 1500;   // our own pickup the host hasn't seen yet
    if(cd > 0){
      if(pad.active && !mineRecently) burst(pad.x,pad.y,pad.big?26:14,'#ffcf4d',pad.big?260:180,0.5,pad.big?4:3,false);
      pad.active = false;
      if(!mineRecently) pad.cd = cd;
    } else if(!pad.active && !mineRecently){
      pad.active = true; pad.cd = 0;
    }
  }
}
function guestPadPickups(S, now){
  for(let i=0; i<boostPads.length; i++){
    const pad = boostPads[i];
    if(!pad.active) continue;
    if(Math.hypot(carRed.x-pad.x, carRed.y-pad.y) < pad.r+24){
      carRed.boost = Math.min(100, carRed.boost + (pad.big?100:25));
      pad.active = false; pad.cd = pad.big ? 10 : 4;
      S.padLocalAt[i] = now;
      S.padLog.push({ q:++S.padSeq, i, at:now });   // tell the host so it disables the pad for everyone
      Audio1.pad(pad.big);
      burst(pad.x,pad.y,pad.big?26:14,'#ffcf4d',pad.big?260:180,0.5,pad.big?4:3,false);
    }
  }
}
function guestRegisterHit(S, now){
  S.hitSeq++;
  S.hitAt = now;
  S.lastHit = { at:now, d:{ q:S.hitSeq, x:r1(ball.x), y:r1(ball.y), z:r1(ball.z), vx:r1(ball.vx), vy:r1(ball.vy), vz:r1(ball.vz) } };
  netPublish(S, true, false);
}
function guestReconcileBall(S, dt, now){
  const snap = S.hostSnap;
  if(!snap || !snap.ball) return;
  if(now < S.predictUntil) return;                                 // we predicted a host touch; wait for the host's version
  if(S.hitSeq > (snap.ha||0) && now - S.hitAt < 1200) return;      // the host hasn't processed our latest touch yet
  const age = (now - S.hostSnapAt)/1000;
  if(age > 0.3) return;                                            // packets stalled: keep predicting rather than chase stale data
  // compare against where the host's ball is NOW, not where it was when the packet left
  const target = extrapolateBall(snap.ball, age + S.rtt/2000);
  const dx = target.x-ball.x, dy = target.y-ball.y, dz = target.z-ball.z;
  if(Math.hypot(dx, dy, dz) > 160){
    setBallFromNet(ball, target, true);
  } else {
    const k = Math.min(1, 6*dt);
    ball.x += dx*k; ball.y += dy*k; ball.z += dz*k;
    ball.vx += (target.vx-ball.vx)*k; ball.vy += (target.vy-ball.vy)*k; ball.vz += (target.vz-ball.vz)*k;
  }
}
function guestUpdate(S, dt){
  const now = performance.now();
  const live = game.state === STATE.PLAYING || game.state === STATE.OVERTIME;
  if(!live){
    // countdown / goal pause: nothing moves locally, show exactly what the host shows
    if(S.hostSnap && S.hostSnap.ball) setBallFromNet(ball, S.hostSnap.ball, true);
    driveRemoteCar(S, carBlue, dt);
    return;
  }
  carRed.update(dt, carRed.localInput(), ball, carBlue);
  guestPadPickups(S, now);
  driveRemoteCar(S, carBlue, dt);
  ball.update(dt);
  if(carBallCollision(carRed, ball)) guestRegisterHit(S, now);
  if(carBallCollision(carBlue, ball)) S.predictUntil = Math.max(S.predictUntil, now + S.rtt + 120);
  carCarCollision(carBlue, carRed);
  guestReconcileBall(S, dt, now);
}
...
function update(dt){
  ...
  if(game.mode==='online'){
    const S = net;
    if(!S || !S.started) return;          // searching, connecting, or finished: nothing to simulate
    if(!netTick(S)) return;               // opponent gone: the match was just torn down
    if(S.role==='guest'){ guestUpdate(S, dt); return; } // the guest mirrors the host's state machine
    // the host falls through and runs the normal state machine below
  }
  ...
  if(game.state===STATE.PLAYING || game.state===STATE.OVERTIME){
    ...
    if(game.mode==='online' && netRole==='host'){
      // The guest's car is dead-reckoned from its packets. The guest reports
      // its own ball touches (applied below), so the host doesn't run that car
      // through local physics or ball collisions.
      driveRemoteCar(net, carRed, dt);
    ...
    ball.update(dt);
    const blueTouched = carBallCollision(carBlue, ball);
    if(game.mode==='online'){
      if(blueTouched) netPublish(net, true, false); // get the host's touch to the guest right away
    } else {
...
function finishMatch(){
  ...
  if(game.mode==='online'){
    ...
    if(S){
      S.over = true;
      netPublish(S, true, true); // push the final score immediately...
      // ...keep repeating it via netTick() for a moment, then leave. Hanging up
      // instantly made the guest see "Opponent Disconnected" instead of the result.
      S.timers.push(setTimeout(()=>{ if(net===S) cleanupOnlineMatch(false); }, 1500));
    }
  } else {
