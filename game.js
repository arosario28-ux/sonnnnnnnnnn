...
/* -------------------------------- Particles ------------------------------ */
let particles = [];
function spawnParticle(p){ particles.push(p); if(particles.length>900) particles.splice(0,particles.length-900); }
...
/* ============================================================================
   ...
   ============================================================================ */
const DEV_EMAIL = 'dev@rocketrush.game';
...
function lerpBallTowardNet(b, snap, dt){
  const k = Math.min(1, 14*dt);
  b.x += (snap.bx-b.x)*k; b.y += (snap.by-b.y)*k; b.z += (snap.bz-b.z)*k;
...
function beginOnlineMatch(){
  $('#lobbyStatus').textContent = 'Match found! Starting\u2026';
  ...
  matchChannel.on('presence', { event:'leave' }, ()=>{ handleOpponentLeft(); });
  matchChannel.subscribe(async (status)=>{
    if(status === 'SUBSCRIBED'){
      await matchChannel.track({ role: netRole });
      showOnly(null);
      updateHUDVisibility();
      if(netRole==='host'){
        beginCountdown();
  ...
  if(game.mode==='online'){
    if(netRole==='host'){
      ...
      if(hostNet && (hostNet.matchState==='playing' || hostNet.matchState==='overtime')){
        ...
        lerpBallTowardNet(ball, hostNet, dt);
      } else if(remoteNet){
...
function drawField(){
  ...
  for(let x=-stripeW; x<WORLD.w+stripeW; x+=stripeW*2){
    ...
    ctx.fillRect(x,0,stripeW,WORLD.h);
  }
  ctx.restore();
  ...
  // center line & circle
  ctx.beginPath();
  ctx.moveTo(WORLD.cx,0); ctx.lineTo(WORLD.cx,WORLD.h);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(WORLD.cx,WORLD.cy,150,0,Math.PI*2);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(WORLD.cx,WORLD.cy,5,0,Math.PI*2);
  ...
  ctx.strokeRect(0, WORLD.cy-boxH/2, boxW, boxH);
  ctx.strokeRect(WORLD.w-boxW, WORLD.cy-boxH/2, boxW, boxH);
  const boxW2=100, boxH2=WORLD.goalHalf*2+20;
  ctx.strokeRect(0, WORLD.cy-boxH2/2, boxW2, boxH2);
  ctx.strokeRect(WORLD.w-boxW2, WORLD.cy-boxH2/2, boxW2, boxH2);

  // team tint near goals
  const tintB = ctx.createLinearGradient(0,0,340,0);
  tintB.addColorStop(0,'rgba(46,163,255,0.16)'); tintB.addColorStop(1,'rgba(46,163,255,0)');
  ctx.fillStyle=tintB; ctx.fillRect(0,0,340,WORLD.h);
  const tintR = ctx.createLinearGradient(WORLD.w,0,WORLD.w-340,0);
  tintR.addColorStop(0,'rgba(255,77,94,0.16)'); tintR.addColorStop(1,'rgba(255,77,94,0)');
  ctx.fillStyle=tintR; ctx.fillRect(WORLD.w-340,0,340,WORLD.h);
}
...
function drawGoals(){
  ['blue','red'].forEach(team=>{
    ...
    // net back
    ctx.fillStyle = isLeft ? 'rgba(46,163,255,0.10)' : 'rgba(255,77,94,0.10)';
    ctx.fillRect(isLeft?-depth:WORLD.w, yMin, depth, yMax-yMin);
    ...
    for(let yy=yMin; yy<=yMax; yy+=14){
      ctx.beginPath(); ctx.moveTo(xEdge, yy); ctx.lineTo(xEdge+dir*depth, yy); ctx.stroke();
    }

    // posts
    ctx.fillStyle = isLeft ? '#2ea3ff' : '#ff4d5e';
    ctx.fillRect(xEdge-3, yMin-8, 6, 16);
    ctx.fillRect(xEdge-3, yMax-8, 6, 16);
    ctx.fillRect(xEdge-4, yMin-8, dir*depth+4*dir, 6);
    ctx.strokeStyle = isLeft?'#2ea3ff':'#ff4d5e';
    ctx.lineWidth=5;
    ctx.beginPath(); ctx.moveTo(xEdge,yMin); ctx.lineTo(xEdge,yMax); ctx.stroke();
  });
}
...
function drawBall(b){
  ...
  const grad = ctx.createRadialGradient(-6,-8,3,0,0,b.r);
  grad.addColorStop(0,'#ffffff');
  grad.addColorStop(0.55,'#dfe7f0');
  grad.addColorStop(1,'#9aa8ba');
  ctx.fillStyle = grad;
...
function drawCar(car, targetCtx){
  ...
  if(car.boosting){
    ...
    const grad = c.createLinearGradient(0,0,-26*flick,0);
    grad.addColorStop(0, withAlpha(flameColor,0.95));
    grad.addColorStop(1, withAlpha(flameColor,0));
    c.fillStyle = grad;
    c.beginPath();
    c.moveTo(0,-6); c.lineTo(-26*flick,0); c.lineTo(0,6);
    c.closePath(); c.fill();
    c.restore();
  }
...
function syncHUD(){
  $('#scoreBlue').textContent = game.score.blue;
  $('#scoreRed').textContent = game.score.red;
  const timerBox = $('#timerBox');
  if(game.overtime){
    timerBox.textContent='OVERTIME';
    timerBox.classList.add('overtime');
  } else {
    timerBox.textContent = fmtTime(game.timeLeft);
    timerBox.classList.remove('overtime');
  }
  $('#boostP1').style.width = Math.max(0,carBlue.boost)+'%';
  $('#boostP2').style.width = Math.max(0,carRed.boost)+'%';
  $('#camBox').innerHTML = (camera.mode==='ball' ? 'CAM: BALL' : 'CAM: CAR') + ' &nbsp;<span class="camKey">C</span>';
}
